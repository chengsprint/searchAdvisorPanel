...
function kpiGrid(items) {
  ...
  items.forEach(function (it) {
    ...
    d.innerHTML = sanitizeHTML(`${iconHtml}<div style="width:100%;font-size:11px;color:var(--sadv-text-tertiary,#8d8d8d);line-height:1.4;margin-bottom:8px;word-break:keep-all;font-weight:600;text-transform:uppercase;letter-spacing:0.04em">${escHtml(it.label)}</div><div style="width:100%;font-size:${valueFontSize};font-weight:650;color:${it.color || C.text};line-height:1.06;letter-spacing:-0.01em;padding-right:4px;word-break:keep-all">${escHtml(it.value)}</div><div style="width:100%;font-size:11px;color:var(--sadv-text-secondary,#c6c6c6);line-height:1.4;margin-top:8px;visibility:${it.sub ? "visible" : "hidden"}">${escHtml(it.sub || "&nbsp;")}</div>`);
    g.appendChild(d);
  });
  return g;
}
